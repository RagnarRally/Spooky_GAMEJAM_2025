...
@export var spawn_interval_resource: Vector2 = Vector2(2,3)
@export var spawn_spread_deg: float = 30
@export var min_distance_between_planets: float = 400
@export var min_distance_between_resources: float = 200
...
func _process(delta: float) -> void:
	...
	if _current_spawn_time_resource <= 0:
		var did_spawn_resource = spawn_resource()
		print("Spawned resource: " + str(did_spawn_resource))
		_current_spawn_time_resource = randf_range(spawn_interval.x, spawn_interval.y)
...
func _valid_resource_placement(new_resource_pos):
	for p in _spawned_planets: #Keep distance from planets
		...
		if d < pow(min_distance_between_resources, 2):
			return false
	return true
